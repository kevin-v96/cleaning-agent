...
class CleaningCompanyTasks:
	def guardrail_task(self, main_task, agent):
        ...
        expected_output=dedent(f"""If the customer's main task - {main_task} - is related to cleaning services, the query is redirected to the Customer Service Assistant.
	...
	def cleaning_service_task(self, main_task, agent):
		return Task(
        description=dedent(f"""Determine whether the task - {main_task} - is regular cleaning, or post-renovation cleaning 
						   ...
						   to a human agent."""),
        expected_output=dedent("""If the task is regular cleaning, output the cleaning service for which the next availability is required.
